...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[BulkDetectLeadStatus] Starting bulk lead status detection...')

    // Obtener todas las conversaciones sin estado de lead
    const { data: conversations, error: convError } = await supabase
      .from('conversations')
      .select('id, contact, user_id')
      .is('lead_status', null)
    ...
    if (!conversations || conversations.length === 0) {
      console.log('[BulkDetectLeadStatus] No conversations without lead status found')
      return new Response(
        JSON.stringify({
          success: true,
    ...
    console.log(`[BulkDetectLeadStatus] Found ${conversations.length} conversations to process`)
    ...
    for (const conversation of conversations) {
      try {
        console.log(`[BulkDetectLeadStatus] Processing conversation: ${conversation.id}`)
        ...
        if (msgError) {
          console.error(`[BulkDetectLeadStatus] Error fetching messages for ${conversation.id}:`, msgError)
          errors++
          continue
        }

        if (!messagesData || messagesData.length === 0) {
          // Si no hay mensajes, marcar como cold por defecto
          ...
          if (updateError) {
            console.error(`[BulkDetectLeadStatus] Error updating ${conversation.id}:`, updateError)
            errors++
          } else {
            processed++
            results.push({
        ...
        console.error(`[BulkDetectLeadStatus] Error processing conversation ${conversation.id}:`, convError)
        errors++
      }
    }

    console.log(`[BulkDetectLeadStatus] Completed. Processed: ${processed}, Errors: ${errors}`)
    ...
    console.error('[BulkDetectLeadStatus] Error:', error)
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message
